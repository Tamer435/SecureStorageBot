...
delete_existing_table = "drop table if exists users"
create_table_query = """
create table users(user_id varchar(20), password varchar(100), salt varchar(100))
"""
...
class CleanStorage(StatesGroup):
    '''
    Машина-состояние, которое отвечает за процесс очистки хранилища
    '''
    getPassword = State()
...
async def start(message: types.message):
    ...
    if str.lower(message.text) == 'изменить пароль':
        if checkUser(message.from_user.id):
            ...
            await ChangePassword.getOldPassword.set()
        else:
            await bot.send_message(message.from_user.id, "У Вас ещё нет хранилища",
                                   reply_markup=kb.kb_add([kb.buttonInstruction, kb.buttonCreateStorage]))
        return
    if str.lower(message.text) == 'очистить хранилище':
        if checkUser(message.from_user.id):
            await bot.send_message(message.from_user.id, "Введите пароль для очистки хранилища",
                                   reply_markup=kb.kb_add([kb.buttonCancel]))
            await CleanStorage.getPassword.set()
        return
    if str.lower(message.text) == 'создать хранилище':
        if not checkUser(message.from_user.id):
            ...
            await bot.send_message(message.from_user.id, "Хранилище уже создано", reply_markup=kb.kb_add(
                [kb.buttonInstruction, kb.buttonOpenStorage, kb.buttonChangePassword, kb.buttonCleanStorage]))
        return
...
@dp.message_handler(state=ChangePassword.getOldPassword)
async def getOldPassword(message: types.message, state: ChangePassword):
    '''
...
async def changePassword(message: types.message, state: ChangePassword):
    ...
    if message.text != '' and len(message.text) >= 8 and str.lower(message.text) != message.text and str.upper(
        ...
        await bot.delete_message(message.from_user.id, message.message_id)
        await state.finish()
        await bot.send_message(message.from_user.id, "Пароль был успешно заменен ✅", reply_markup=kb.kb_add(
            [kb.buttonInstruction, kb.buttonOpenStorage, kb.buttonChangePassword, kb.buttonCleanStorage]))
    elif str.lower(message.text) == 'отмена':
        await bot.send_message(message.from_user.id, "Смена пароля отменена ❌", reply_markup=kb.kb_add(
            [kb.buttonInstruction, kb.buttonOpenStorage, kb.buttonChangePassword,
...
@dp.message_handler(state=CleanStorage.getPassword)
async def cleanStorage(message: types.message, state: CleanStorage):
    '''
    Функция отвечает за очистку массива id сообщений пользователя.
    :param message: Сообщения пользователя типа types.message
    :param state: Машина состояний типа CleanStorage
    :return: None
    '''
    if comparePasswords(message.text, message.from_user.id):
        getUser(message.from_user.id).messages_id.clear()
        await bot.send_message(message.from_user.id, "Хранилище очищено ✅", reply_markup=kb.kb_add(
            [kb.buttonInstruction, kb.buttonOpenStorage, kb.buttonChangePassword,
             kb.buttonCleanStorage]))
        await state.finish()
    elif str.lower(message.text) == 'отмена':
        await bot.send_message(message.from_user.id, "Очистка отменена", reply_markup=kb.kb_add(
            [kb.buttonInstruction, kb.buttonOpenStorage, kb.buttonChangePassword,
             kb.buttonCleanStorage]))
        await state.finish()
    else:
